...
test('equal returns false when either side has a literal-backslash authority', (t) => {
  // The allowlist surface from the advisory: "http://evil.com\@allowed.com" must
  // neither compare equal to the allowlisted host nor to the URI Node's WHATWG
  // parser would actually fetch ("http://evil.com/@allowed.com").
  const pairs = [
    ['http://evil.com\\@allowed.com', 'http://allowed.com/'],
